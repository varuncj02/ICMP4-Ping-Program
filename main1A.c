...
char* dns_lookup(char* addr_host, struct sockaddr* addr_con)
{
	printf("\nResolving DNS..\n");
	struct addrinfo* host_entity;
	char* ip = (char*)malloc(NI_MAXHOST * sizeof(char));
	int i;
	host_entity = Host_serv(addr_host, NULL, 0, 0);

	if ((i = getaddrinfo(addr_host, PORT_NO, host_entity, host_entity)) == 0)
	{
	...
	strcpy(ip, inet_ntoa(*(struct in_addr*)
		host_entity->ai_addr));
	*addr_con = *host_entity->ai_addr;

	/*(*addr_con).sin_family = host_entity->sin_addrtype;
...
/*	printf("\nReverse Lookup domain: %s",
	...
	send_ping(sockfd, &addr_con, resolved_addr, argv[1]);
